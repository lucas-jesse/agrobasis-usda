...
def variacao(base, attr):
    ...
    atual, ant = s["Value"].iloc[-1], s["Value"].iloc[-2]
    if ant == 0 or pd.isna(ant):
        return "Sem comparação"
    var = ((atual / ant) - 1) * 100
    sinal = "+" if var >= 0 else ""
...
def cagr(base, attr):
    ...
    if len(s) < 2:
        return None
    ini, fim = s["Value"].iloc[0], s["Value"].iloc[-1]
...
tabs = st.tabs(["📈 Visão Executiva", "⚖️ Balanço", "🌍 Market Share", "🏆 Rankings", "📊 Diagnóstico", "📋 Dados"])
...
with tabs[0]:
    ...
    with col_b:
        ...
        <div class="insight-box">
            ...
            <p>Este painel mostra a trajetória do indicador selecionado e sua relação com os principais fundamentos: produção, consumo, esmagamento, exportações e estoques.</p>
            <p>Para análise de preço, acompanhe principalmente estoque/uso, ritmo de exportação, crescimento do esmagamento e concentração do market share global.</p>
        </div>
        """, unsafe_allow_html=True)
