...
def aplicar_rotulos_linha(fig, casas=1, percentual=False):
    """Deixa linhas prontas para imagem estática/PPT, com valores visíveis."""
    sufixo = "%" if percentual else ""
    template = f"%{{y:,.{casas}f}}{sufixo}"
    fig.update_traces(
        ...
        line=dict(width=4),
        marker=dict(size=9, line=dict(width=1.4, color="#ffffff")),
        cliponaxis=False
    )
    return fig


def aplicar_rotulos_barra(fig, casas=1, percentual=False, orientacao="v"):
    """Adiciona valores nas barras para melhorar o uso em imagens baixadas."""
    sufixo = "%" if percentual else ""
    eixo = "x" if orientacao == "h" else "y"
    fig.update_traces(
        texttemplate=f"%{{{eixo}:,.{casas}f}}{sufixo}",
...
def aplicar_layout(fig, h=500, fonte="Fonte: USDA PSD • Elaboração: AgroBasis"):
    ...
    if os.path.exists(logo_path):
        ...
        fig.add_annotation(
            ...
            x=0.5, y=0.5,
            showarrow=False,
            font=dict(size=72, color="rgba(20,83,45,0.065)"),
            xanchor="center",
            yanchor="middle"
        )
...
with tabs[0]:
    ...
    with col_a:
        ...
        aplicar_rotulos_linha(fig, casas=1)
    ...
    aplicar_rotulos_linha(fig_comp, casas=1)
...
with tabs[1]:
    ...
    aplicar_rotulos_linha(fig_bal, casas=1)
    ...
    with col1:
        if "Ending Stocks" in pivot.columns and "Domestic Consumption" in pivot.columns:
            ...
            aplicar_rotulos_barra(fig_su, casas=1, percentual=True)
    ...
    with col2:
        if {"Production", "Domestic Consumption"}.issubset(pivot.columns):
            ...
            aplicar_rotulos_barra(fig_gap, casas=1)
...
with tabs[4]:
    ...
    with c1:
        if {"Exports", "Production"}.issubset(pivot.columns):
            ...
            aplicar_rotulos_linha(fig, casas=1, percentual=True)
...
with tabs[5]:
    ...
    aplicar_rotulos_barra(fig_yoy, casas=1, percentual=True)
